...
%Suppose a researcher uses maximum likelihood to estimate a statistical model in which $y_i \stackrel{iid}{\sim} f(\theta_i)$, where $i \in \{1,\ldots, N\}$ and $f$ represents a probability distribution. The parameter $\theta_i$ is connected to a design matrix $X$ of $k$ explanatory variables and a column of ones by a link function $g$, so that $g(\theta_i) = X_i\beta$, where $\beta \in \mathbb{R}^{k+1}$ represents a vector of coefficients with length $k + 1$. The researcher uses maximum likelihood to compute estimates $\hat{\beta}^{\text{mle}}$ for the parameter vector $\beta$.\footnote{Note that we adopt a frequentist perspective, so $\hat{\beta}^{\text{mle}}$ represents a \emph{random variable} that varies across samples.}

%While it might seem obvious that when estimating non-linear models, estimated model coefficients are often of limited interest, the publication of \cite{KingTomzWittenberg2000} made this point very clear, admonishing political scientists to focus on substantively meaningful {\it quantities of interest} when reporting their empirical results. For example, when fitting binary, ordinal, or multinomial probit models, we typically care about probabilities or changes in probabilities (i.e., marginal effects or first differences). When modeling count data using Poisson or negative binomial models, we often are most interested in the expected count or changes in the expected count.

%\noindent It would be impossible to overstate the salutary effects that \cite{KingTomzWittenberg2000} has had on quantitative political science research. Before its publication, it was common for political scientists to present lengthy tables of estimated model coefficients from non-linear models, with little or no attention paid to the substantive interpretation of the results beyond the sign and statistical significance of the model coefficients. When researchers did report quantities of interest these were typically not accompanied by measures of uncertainty. Expectations about how empirical results should be presented changed swiftly after the publication of \cite{KingTomzWittenberg2000}. According to the Web of Science, by January 2018 \cite{KingTomzWittenberg2000} had been cited 1,097 times, making it the fourth most-cited methodology article in political science, at least among those published in the \textit{American Political Science Review}, the \textit{American Journal of Political Science}, and \textit{Political Analysis}. Moreover, it is the second most-cited article \textit{overall} in the \textit{American Journal of Political Science}, falling just $56$ citations short of \cite{BeckKatzTucker1998}. Google Scholar suggests that \cite{KingTomzWittenberg2000} has as many as 3,598 citations and that \cite{TomzWittenbergKing2003}, which provides an overview of the CLARIFY software, has as many as 1,437. In short, the stochastic simulation approach that \cite{KingTomzWittenberg2000} advocates has been incredibly influential, both in political science and in neighboring disciplines.
...
%The ML invariance property does not come without cost.
...
%\footnote{One could use alternative approaches to generate $\tilde{\beta}$, such as the parametric or non-parametric bootstrap. As long as $\tilde{\beta}$ approximately follows a normal distribution the intuitions and best practices we offer below still apply.}[HOLGER: I'm not sure we need this footnote. I think it might make more sense to have a sentence towards the end pointing out that statistical inference is possible using various commonly used approaches]
...
%Second, the invariance property is tied to maximum likelihood estimation. While most estimators used in political science research are maximum likelihood estimators, increasingly, researchers are using tools from statistical learning that are not rooted in parametric assumptions about the distribution of the outcome variable. Examples include tree-based learners such as bagging, boosting, and random forest (e.g., ). How should we approach the estimation of quantities of interests from such model fits?
...
%When we average the simulation averages (as Hanmer and Kalkan do in their Stata code), we obtain an estimated effect of 0.23. The maximum likelihood estimate is about 22\% larger than the simulation average--the choice matters.
